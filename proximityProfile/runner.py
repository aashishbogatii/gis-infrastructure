...
Run:  python -m riskProfile.runner <APN>
"""
...
def _parcel_sql() -> str:
    """The by-APN parcel lookup (one bind param: the normalized APN)."""
    return (
        f"SELECT geometry, parcel_apn, countyname, fullstreetaddress "
        f"FROM read_parquet('{backends.PARCEL_PATH}') "
        f"WHERE REPLACE(parcel_apn, '-', '') = ? LIMIT 1"
    )


def fetch_parcel_row(con, apn: str) -> bytes:
    """The parcel geometry (WKB) for a normalized APN, or raise if not found."""
    row = con.execute(_parcel_sql(), [normalize_apn(apn)]).fetchone()
    if row is None:
...
if __name__ == "__main__":
    setup_logging()
    p = argparse.ArgumentParser(prog="riskProfile.runner")
    p.add_argument("apn")
    args = p.parse_args()

    t = time.perf_counter()
    result = proximity(args.apn)
